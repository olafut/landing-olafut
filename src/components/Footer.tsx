...
export const Footer = () => {
  const { t } = useTranslate('Layout.Footer');
    ...
    <footer className="relative bg-foreground dark:bg-neutral-800 text-background dark:text-foreground mt-16 overflow-hidden rounded-t-5xl font-sans">
      <div className="container mx-auto px-6 pt-16 pb-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 pb-16">
          <BrandSection />
          <NavigationSection title={t('navigation.title')} />
          <SocialSection />
          <ContactSection />
        </div>
...
function SocialSection() {
  ...
  return (
    ...
    >
      <h3 className="text-lg font-bold mb-6">{t('social.title')}</h3>
...
function NavigationSection({ title }: NavigationSectionProps) {
